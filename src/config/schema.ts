...
export const BridgeConfigSchema: z<BridgePluginConfig> = z.object({
  ...
  group_memory_budget_chars: z.number().default(DEFAULT_GROUP_MEMORY_BUDGET_CHARS).description('群聊记忆预算字符上限：控制群规与多位活跃群友画像注入 Prompt 的总预算，兼单群规则写入上限 (默认 2200)'),
  private_memory_budget_chars: z.number().default(DEFAULT_PRIVATE_MEMORY_BUDGET_CHARS).description('私聊记忆预算字符上限：控制私聊单用户画像及私聊规则注入与存储的字符预算上限 (默认 1500)'),
  memory_budget_chars: z.number().default(DEFAULT_MEMORY_BUDGET_CHARS).description('兼容旧配置：群聊用户画像注入总字符预算上限 (默认 2200)'),
