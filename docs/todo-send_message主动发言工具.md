...
- [ ] 2.1 新增动态段（order ~10 最前，仅 QQ 会话，非 QQ 空串）
- [ ] 2.2 段文本 = §2.2 定稿
- [ ] 2.3 契约测试：QQ 会话含该段 / 非 QQ 为空 / order 最前

## 阶段 3：send_message 工具

- [ ] 3.1 工具实现（单 text 参数、走串行队列 + stripMarkdown、超长分段、返回 message_id）
- [ ] 3.2 注册范围：仅普通 QQ 会话（群+私）；沙箱/非 QQ 不注册
- [ ] 3.3 报错三分支 + 防御守卫；工具描述 §1.5
- [ ] 3.4 契约测试（注册范围/报错/成功/队列）

## 阶段 4：出站旁白抑制 + turn/end 兜底

- [ ] 4.1 与 tool-call 同块文本永不自动发送（结构抑制）
- [ ] 4.2 每轮 send_message 计数；0 次 → turn/end 补发终答（路径与现状一致）；≥1 → 不补发
- [ ] 4.3 契约测试：旁白不发出 / 兜底两分支

## 阶段 5：send_message 首调引用规则

- [ ] 5.1 一轮多次 send_message：仅首次带锚定消息引用/艾特前缀，后续纯文本
- [ ] 5.2 单次调用行为不变
- [ ] 5.3 契约测试：多调场景仅首条带前缀

## 阶段 6：收尾验收

- [ ] 6.1 全量 pnpm test 绿 + pnpm build 通过 + dist 含改动
- [ ] 6.2 逐项对照 Checklist A-F 勾选；未做项空着
- [ ] 6.3 汇报：调研纪要、锚定 bug 根因与修复、实现项、测试/构建、真机待验项