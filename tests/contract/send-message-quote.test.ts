/**
 * 契约测试: 出站直发模式首调引用规则 (First-Call Quote/At Rule)
 *
 * 覆盖规范清单:
 * 契约 1: 一轮内多次输出正文：第 1 次携带引用 (reply) 与艾特 (at) 前缀，第 2 次及后续为纯文本，不重复携带引用/艾特；
 * 契约 2: 单次输出场景：本轮仅输出 1 次，正常携带引用与艾特前缀；
 * 契约 3: 单次超长消息分段：首次输出超长分段下发时，仅第 1 个分段携带前缀，后续分段均为纯文本；
 * 契约 4: 私聊会话隔离：私聊会话 (qq-user-* / user_*) 下无论是首次还是多次，均下发纯文本，不引用不艾特；
 * 契约 5: 跨轮次隔离：Turn 1 结束后开启 Turn 2，Turn 2 的首次出站能够再次正常携带 Turn 2 的锚点引用；
 * 契约 6: 真实生产装配闭环：使用 bootDshNapcatBridge + 真实 WebSocket 网关，端到端验证群消息入站后，一轮内模型多次输出中间/终答时真实出站 WebSocket 消息帧的首调前缀与后续纯文本行为。
 */
...
describe('契约 1 ~ 5: 直发模式首调引用规则单元契约', () => {
  ...
  it('契约 1: 一轮内多次输出正文：第 1 次输出携带引用与艾特前缀，第 2 次及后续输出为纯文本', async () => {
    const { bridge, sent } = setupBridgeAndGateway({ quote_original: true, at_questioner: true });
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 1,
        message: { content: [{ type: 'text', text: '正在查询天气情况，请稍候...' }] },
      },
    } as any);
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 2,
        message: { content: [{ type: 'text', text: '查询完成：今日晴朗，25℃。' }] },
      },
    } as any);

    expect(sent).toHaveLength(2);

    // 第 2 次输出必须是纯文本，不再重复携带 reply 与 at
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 3,
        message: { content: [{ type: 'text', text: '如有其他问题随时询问。' }] },
      },
    } as any);
  ...
  it('契约 2: 单次调用场景：本轮仅调用 1 次输出，正常携带引用与艾特前缀', async () => {
    const { bridge, sent } = setupBridgeAndGateway({ quote_original: true, at_questioner: true });
    const session = { id: 'qq-group-10001' };
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 1,
        message: { content: [{ type: 'text', text: '单次处理完成，这是直接给出的答复。' }] },
      },
    } as any);
  ...
  it('契约 3: 首次输出多段分块下发时，仅第 1 个分段携带前缀，后续分段均为纯文本', async () => {
    const { bridge, sent } = setupBridgeAndGateway({ quote_original: true, at_questioner: true });
    const session = { id: 'qq-group-10001' };
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        ...
        message: {
          content: [
            { type: 'text', text: '第一段：关于总体运行状态分析...' },
            { type: 'text', text: '第二段：关于详细性能优化建议...' },
          ],
        },
      },
    } as any);
    ...
    expect(firstChunkMsg.find((s: any) => s.type === 'text')?.data?.text).toBe('第一段：关于总体运行状态分析...');

    // 后续分段必须为纯文本，绝不重复携带 reply 与 at
    const secondChunkMsg = sent[1].msg;
    expect(secondChunkMsg).toBe('第二段：关于详细性能优化建议...');
  });

  it('契约 4: 私聊会话隔离：私聊会话下无论是首次还是多次输出，均下发纯文本，不引用不艾特', async () => {
    const { bridge, sent } = setupBridgeAndGateway({ quote_original: true, at_questioner: true });
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 1,
        message: { content: [{ type: 'text', text: '私聊首次回复' }] },
      },
    } as any);
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 2,
        message: { content: [{ type: 'text', text: '私聊后续回复' }] },
      },
    } as any);
  ...
  it('契约 5: 跨轮次隔离：Turn 1 结束后开启 Turn 2，Turn 2 的首次出站能够再次正常携带 Turn 2 的锚点引用', async () => {
    const { bridge, sent } = setupBridgeAndGateway({ quote_original: true, at_questioner: true });
    const session = { id: 'qq-group-10001' };
    ...
    // Turn 1 第 1 次: 带 Turn 1 前缀
    await bridge.handleSessionEvent(session as any, {
      type: 'assistant/message',
      data: {
        turn: 1,
        step: 1,
        message: { content: [{ type: 'text', text: 'Turn 1 进度汇报' }] },
      },
    } as any);
    ...
    await bridge.handleSessionEvent(session as any, {
      ...
      data: {
        turn: 1,
        step: 2,
        message: { content: [{ type: 'text', text: 'Turn 1 终答' }] },
      },
    } as any);
    ...
    // Turn 2 第 1 次: 必须携带 Turn 2 的新锚点引用 (msg_id: 50002) 与 at 提问者
    await bridge.handleSessionEvent(session as any, {
      type: 'assistant/message',
      data: {
        turn: 2,
        step: 1,
        message: { content: [{ type: 'text', text: 'Turn 2 进度汇报' }] },
      },
    } as any);
    ...
    // Turn 2 第 2 次: 再次变为纯文本
    await bridge.handleSessionEvent(session as any, {
      type: 'assistant/message',
      data: {
        turn: 2,
        step: 2,
        message: { content: [{ type: 'text', text: 'Turn 2 终答' }] },
      },
    } as any);
...
describe('契约 6: 真实生产装配闭环 (Real Assembly Contract via bootDshNapcatBridge + WS Gateway)', () => {
  ...
  it('真实装配下：群消息入站后，一轮内模型多次输出中间/终答时真实出站 WebSocket 消息帧的首调前缀与后续纯文本行为', async () => {
    ...
    (booted.ctx as any).emit('session/event', session, {
      ...
      data: {
        turn: 1,
        step: 1,
        message: { content: [{ type: 'text', text: '正在生成报表，请稍等片刻...' }] },
      },
    });
    ...
    (booted.ctx as any).emit('session/event', session, {
      ...
      data: {
        turn: 1,
        step: 2,
        message: { content: [{ type: 'text', text: '报表已生成完毕，今日总访问量 1024。' }] },
      },
    });
